...
export default function ClientDetailPage() {
  ...
  const loadClientInfo = useCallback(async () => {
    try {
      ...
      const { data: userRaw, error: userError } = await supabase
        .from("users")
        .select("name, email, client_identifier, has_auth_access")
      ...
      setClientInfo({
        ...
        user: {
          ...
          has_auth_access: user.has_auth_access,
        },
      });
    } catch (err) {
  ...
  async function handleCreateTask(e: React.FormEvent) {
    ...
    try {
      ...
      // Get coach's user record (using auth_user_id)
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-lg font-medium text-gray-900 font-mono text-sm">
              {clientInfo.user.email || clientInfo.user.client_identifier || "N/A"}
            </p>
          </div>
          <div>
